...
In Azure DevOps, Frogbot automatically scans newly opened pull requests without waiting for maintainer approval. As a result, using Frogbot with Azure DevOps is not recommended for open-source projects.
...
5. Select the repository in which the Frogbot pipelines will reside.
6. Select `Starter Pipeline` and name it `frogbot`.
7. Use the content of the templates below for the pipeline.&#x20;
8. Edit the remaining mandatory `Variables`.
...
8. (Optional) To set a pull request trigger in Azure Repos, add a branch policy via **Branch Policies**:
   1. Go to **Branches**.
   2. For each branch you wish Frogbot to scan pull requests, from the menu select **Branch policies**.
   3. Add a new build policy.
   4. Fill in the build policy parameters and select **Save**.
...
{% hint style="info" %}
You can also use `JF_XRAY_URL` and `JF_ARTIFACTORY_URL` instead of `JF_URL`, and `JF_ACCESS_TOKEN` instead of `JF_USER` and `JF_PASSWORD`.
{% endhint %}

10. To set the `Variables` in the pipeline edit page, click on the `Variables` button and set the `Variables`.
