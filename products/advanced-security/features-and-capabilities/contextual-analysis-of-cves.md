...
* **Upgrade Required**: _(Primarily for Self-Hosted)_ The Xray version needs to be updated to receive a new scanner for this CVE. A rescan is required after the upgrade is complete. _(SaaS)_ In rare cases, SaaS customers might see this status before the new scanner version is fully deployed to the environment. No action is required from the customer side—the scanner will operate automatically once the update is available.
